...
**Resolved 2026-07-31 via a substitute-standard justification**, per the
manuscript's own stated fallback rule: **ДСТУ Б EN 15251:2011**
...
is built from) has its own Таблиця А.2 ("Examples of recommended design
values of the indoor temperature for design of buildings and HVAC
systems"), which names **кухні (kitchens)** explicitly in a residential
row alongside bedrooms/living rooms, with real Category I/II/III
...
favorable/acceptable/degraded/critical schema the same way DBN's
...
grouping either, independently corroborating DBN's own gap rather than
filling it — Table A.2 is the structurally correct analogue to DBN's Table
Д.4 (design/comfort bands, not hourly simulation ranges), and is the one
actually used.
...
DBN-row substitution, never fabrication. In **strict `mode=publication`**
(the default) that error still aborts the entire pipeline run for any
genuinely-undefined combination. An optional **`mode=exploratory`**
structurally omits the microclimate (M) component instead (never
fabricates it) for whichever combinations remain undefined, and tags the
run so it can never be promoted into `research_results/final`.
...
- **The kitchen/warm_period microclimate component is sourced to a
  ...
  used is instead cited to ДСТУ Б EN 15251:2011 Table A.2 (§10), per the
  manuscript's own stated fallback rule. This is a real, verified,
  standards-based citation — not a fabrication or a different-room
  substitution — but readers should know the warm-period microclimate
  component rests on a different (EN-derived, Ukrainian-adopted) standard
  than the cold-period one, which is direct DBN. `research_results/final/`
  discloses this explicitly wherever it affects a claim.
