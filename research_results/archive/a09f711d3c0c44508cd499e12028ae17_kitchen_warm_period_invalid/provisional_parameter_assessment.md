...
**Two hard rules enforced throughout:** a synthetic benchmark's calibration result is scoped to that benchmark and is never claimed to make a parameter universally valid; no parameter is promoted from PROVISIONAL to STANDARD_BASED in this report -- that only happens in `iaq_hfis.provenance`'s own catalog, with a cited source, never here.

16 PROVISIONAL parameter(s) in the current configuration; 16 engaged (actually applied) this run.
...
- **Engaged this run**: yes
- **Why provisional**: The manuscript does not specify PARTIAL-mode inference mechanics; see fuzzy_engine.infer_index docstring for why naive filtering of the full rule base would be unsound.
...
- **Current value**: `1.0`
- **Engaged this run**: yes
- **Why provisional**: Same source as hampel.window_size (t=1 in the cited paper's example).
...
- **Engaged this run**: yes
- **Why provisional**: Pearson, Neuvo, Astola, Gabbouj, "Generalized Hampel Filters" (2016) -- the manuscript's own cited source's illustrative-example parameters (K=5 -> 11-point window); the paper states this as a worked example, not a general recommendation.
