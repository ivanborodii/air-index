...
Run `7e71d6bc86f142cf87a26b0e314bcb5b` computed the hierarchical fuzzy indoor air quality index over 2026-06-18T00:00:00+00:00 to 2026-08-04T16:24:02+00:00, using a 15-minute rolling window, recomputed at each aligned timestamp (13732 timestamps processed).
...
## Completeness
Of 13732 computed timestamps, 12212 were OK (all three components available with sufficient coverage), 775 were PARTIAL (one component unavailable), and 745 were FAILED (index and class not formed).
Dominant-component frequency across OK/PARTIAL computed timestamps: A: 4282 (33.0%), M: 7859 (60.5%), V: 846 (6.5%).

## Method comparison
Evaluation run `0bc42ce1129e45b78e8cfcfc3912baea` (the selected evaluation for this pipeline run):
- CRISP_CLASS_MAX and FUZZY_COMPONENT_MAX agreed on 99.8% of 13157 compared timestamps (unlabeled agreement, Cohen's kappa=0.998).
- CRISP_CLASS_MAX and PROPOSED_HFIS agreed on 99.8% of 12987 compared timestamps (unlabeled agreement, Cohen's kappa=0.998).
- CRISP_CLASS_MAX and WEIGHTED_MEAN agreed on 31.8% of 13157 compared timestamps (unlabeled agreement, Cohen's kappa=0.144).
- FUZZY_COMPONENT_MAX and PROPOSED_HFIS agreed on 100.0% of 12987 compared timestamps (unlabeled agreement, Cohen's kappa=1.000).
- FUZZY_COMPONENT_MAX and WEIGHTED_MEAN agreed on 31.7% of 13157 compared timestamps (unlabeled agreement, Cohen's kappa=0.142).
- PROPOSED_HFIS and WEIGHTED_MEAN agreed on 30.8% of 12987 compared timestamps (unlabeled agreement, Cohen's kappa=0.135).
- FUZZY_COMPONENT_MAX hid a component that individually reached Critical in 0.0% of 5059 such events.
- CRISP_CLASS_MAX hid a component that individually reached Critical in 0.0% of 5059 such events.
- WEIGHTED_MEAN hid a component that individually reached Critical in 98.4% of 5059 such events.
...
- CRISP_CLASS_MAX: class changed in 34.6% of 900 trials, 95% CI [31.5%, 37.7%] (moved to a strictly better class in 10.1% of trials, a strictly worse class in 24.4% -- these two sum to the class-change rate); mean absolute index change 8.64, p95 25.00.
- FUZZY_COMPONENT_MAX: class changed in 34.0% of 900 trials, 95% CI [31.0%, 37.2%] (moved to a strictly better class in 9.8% of trials, a strictly worse class in 24.2% -- these two sum to the class-change rate); mean absolute index change 7.15, p95 17.03.
- PROPOSED_HFIS: class changed in 34.0% of 900 trials, 95% CI [31.0%, 37.2%] (moved to a strictly better class in 9.8% of trials, a strictly worse class in 24.2% -- these two sum to the class-change rate); mean absolute index change 7.15, p95 17.03.
- WEIGHTED_MEAN: class changed in 6.0% of 900 trials, 95% CI [4.6%, 7.7%] (moved to a strictly better class in 1.3% of trials, a strictly worse class in 4.7% -- these two sum to the class-change rate); mean absolute index change 3.07, p95 7.12.
...
- single_spike (row-level): precision=0.104, recall=1.000, F1=0.189 (tp=7, fp=60, fn=0).
- stuck_value (row-level): precision=1.000, recall=1.000, F1=1.000 (tp=32, fp=0, fn=0).
- data_loss (row-level): precision=1.000, recall=1.000, F1=1.000 (tp=20, fp=0, fn=0).
- gradual_drift (row-level): precision=0.615, recall=0.600, F1=0.608 (tp=24, fp=15, fn=16).
- out_of_range (row-level): precision=1.000, recall=1.000, F1=1.000 (tp=5, fp=0, fn=0).
- single_spike (event-level, tolerance=1 samples): precision=0.333, recall=1.000, F1=0.500 (7 true event(s), 21 predicted event(s)).
- stuck_value (event-level, tolerance=1 samples): precision=1.000, recall=1.000, F1=1.000 (4 true event(s), 4 predicted event(s)).
- data_loss (event-level, tolerance=1 samples): precision=1.000, recall=1.000, F1=1.000 (4 true event(s), 4 predicted event(s)).
- gradual_drift (event-level, tolerance=1 samples): precision=0.400, recall=1.000, F1=0.571 (4 true event(s), 10 predicted event(s)).
- out_of_range (event-level, tolerance=1 samples): precision=1.000, recall=1.000, F1=1.000 (5 true event(s), 5 predicted event(s)).
- **Disclosed limitation**: single_spike scored row-level F1 below 0.5 on this benchmark -- reported here as-is, not hidden or excluded from the summary.
- Genuine sustained events falsely rejected: 0.0%.
- Hampel calibration grid (window_size x mad_multiplier) evaluated on calibration and validation scenario splits; mad_multiplier=3.0 was selected purely from the calibration split at window_size=11 (configured value matches selection: True) -- see parameter_selection.json for the full deterministic procedure, hampel_calibration.csv for the full grid, and 'Provisional parameters engaged' above.
...
- Fault-injection precision is weak for single_spike on this run's validation split (F1 below 0.5) -- disclosed here, not excluded from the summary above.
