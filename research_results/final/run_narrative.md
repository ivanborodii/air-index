...
Run `ea2cb02424ef4be99374f0f40a41d497` computed the hierarchical fuzzy indoor air quality index over 2026-07-15T00:00:00+00:00 to 2026-07-29T00:00:00+00:00, using a 15-minute rolling window, recomputed at each aligned timestamp (4032 timestamps processed).
...
Dominant-component frequency across OK/PARTIAL computed timestamps: A: 1536 (38.2%), M: 2306 (57.4%), V: 175 (4.4%).

## Method comparison
Evaluation run `0348eeaf3b5543818f2b3bf45bc89672` (the selected evaluation for this pipeline run):
- CRISP_CLASS_MAX and FUZZY_COMPONENT_MAX agreed on 99.8% of 4032 compared timestamps (unlabeled agreement, Cohen's kappa=0.997).
- CRISP_CLASS_MAX and PROPOSED_HFIS agreed on 99.8% of 4017 compared timestamps (unlabeled agreement, Cohen's kappa=0.997).
- CRISP_CLASS_MAX and WEIGHTED_MEAN agreed on 42.7% of 4032 compared timestamps (unlabeled agreement, Cohen's kappa=0.171).
- FUZZY_COMPONENT_MAX and PROPOSED_HFIS agreed on 100.0% of 4017 compared timestamps (unlabeled agreement, Cohen's kappa=1.000).
- FUZZY_COMPONENT_MAX and WEIGHTED_MEAN agreed on 42.5% of 4032 compared timestamps (unlabeled agreement, Cohen's kappa=0.170).
- PROPOSED_HFIS and WEIGHTED_MEAN agreed on 42.3% of 4017 compared timestamps (unlabeled agreement, Cohen's kappa=0.168).
- FUZZY_COMPONENT_MAX hid a component that individually reached Critical in 0.0% of 755 such events.
- CRISP_CLASS_MAX hid a component that individually reached Critical in 0.0% of 755 such events.
- WEIGHTED_MEAN hid a component that individually reached Critical in 99.1% of 755 such events.
...
- CRISP_CLASS_MAX: class changed in 38.1% of 900 trials, 95% CI [35.0%, 41.3%] (moved to a strictly better class in 12.6% of trials, a strictly worse class in 25.6% -- these two sum to the class-change rate); mean absolute index change 9.53, p95 25.00.
- FUZZY_COMPONENT_MAX: class changed in 36.8% of 900 trials, 95% CI [33.7%, 40.0%] (moved to a strictly better class in 15.3% of trials, a strictly worse class in 21.4% -- these two sum to the class-change rate); mean absolute index change 7.61, p95 17.79.
- PROPOSED_HFIS: class changed in 36.8% of 900 trials, 95% CI [33.7%, 40.0%] (moved to a strictly better class in 15.3% of trials, a strictly worse class in 21.4% -- these two sum to the class-change rate); mean absolute index change 7.58, p95 17.59.
- WEIGHTED_MEAN: class changed in 7.2% of 900 trials, 95% CI [5.7%, 9.1%] (moved to a strictly better class in 3.1% of trials, a strictly worse class in 4.1% -- these two sum to the class-change rate); mean absolute index change 3.11, p95 7.23.
...
## Boundary continuity
Deterministic input grids (21 points each) around 33 control-region boundaries, each swept under 3 'other components' contexts (acceptable, degraded, favorable), comparing PROPOSED_HFIS, FUZZY_COMPONENT_MAX, and WEIGHTED_MEAN numerically (see continuity_grid.csv / continuity_summary.csv).
