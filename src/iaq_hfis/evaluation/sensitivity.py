...
def sweep_window_minutes(ctx, source: AirMonitorSource, computed_ts: datetime, window_minutes_list: list[int]) -> list[SensitivityResult]:
    ...
    for window_minutes in window_minutes_list:
        outcome = compute_index_at(ctx, source, computed_ts, window_minutes, writer=None)
        results.append(
            SensitivityResult(
...
def sweep_coverage_thresholds(
    ...
    settings variant), unlike the window sweep above."""
    ...
    for threshold in thresholds:
        ...
        outcome = compute_index_at(ctx_variant, source, computed_ts, window_minutes, writer=None)
        results.append(
            SensitivityResult(
