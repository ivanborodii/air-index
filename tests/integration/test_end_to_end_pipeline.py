...
# which is now sourced to a substitute standard (DSTU B EN 15251:2011).
...
def test_publication_mode_succeeds_for_warm_period_kitchen_run(run_and_inspect):
    # kitchen/warm_period is now sourced to DSTU B EN 15251:2011 Table A.2 (a
    # substitute standard, per the manuscript's own stated fallback rule --
    # DBN Table D.4 itself still has no value for this room/season). This is
    # a confirmed (non-provisional) profile, so publication mode must now
    # produce a full A/V/M/I OK-completeness result instead of aborting.
