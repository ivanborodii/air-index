...
def _points(boundary: ContinuityBoundary, method: str, values: list[float | None], classes: list[str | None], context: str = "favourable") -> list[GridPoint]:
    return [
        GridPoint(
...
def test_summarize_boundary_detects_a_planted_hard_discontinuity():
    ...
    smooth_classes = ["Favourable"] * 10 + ["Acceptable"] * 11
    ...
    smooth_summary = _by_method(summarize_boundary(boundary, "favourable", smooth_points))["PROPOSED_HFIS"]
    hard_summary = _by_method(summarize_boundary(boundary, "favourable", hard_points))["FUZZY_COMPONENT_MAX"]
...
def test_summarize_boundary_detects_monotonicity_violation_for_pollutant_channel():
    ...
    classes = ["Favourable"] * 6
    points = _points(boundary, "PROPOSED_HFIS", values, classes)
    summary = _by_method(summarize_boundary(boundary, "favourable", points))["PROPOSED_HFIS"]
...
def test_summarize_boundary_counts_class_transitions_and_positions():
    ...
    classes = ["Favourable", "Favourable", "Favourable", "Acceptable", "Acceptable"]
    points = _points(boundary, "PROPOSED_HFIS", values, classes)
    summary = _by_method(summarize_boundary(boundary, "favourable", points))["PROPOSED_HFIS"]
...
def test_summarize_boundary_computes_local_lipschitz_ratio():
    ...
    classes = ["Favourable"] * 5
    points = [
        GridPoint(boundary_id=boundary.boundary_id, channel=boundary.channel, context="favourable", boundary_value=boundary.boundary_value,
...
def test_summarize_boundary_area_between_curves_only_populated_for_hfis():
    ...
    by_method = _by_method(summarize_boundary(boundary, "favourable", points))
...
def _points_with_grid(boundary: ContinuityBoundary, method: str, inputs: list[float], values: list[float], context: str = "favourable") -> list[GridPoint]:
    ...
    return [
        GridPoint(boundary_id=boundary.boundary_id, channel=boundary.channel, context=context, boundary_value=boundary.boundary_value,
                  grid_index=i, input_value=x, method=method, index_value=v, index_class="Favourable")
...
def test_summarize_continuity_smoothness_reports_hfis_smoother():
    ...
    inputs = [0.0, 1.0, 2.0]
    smooth = _by_method(summarize_boundary(boundary, "favourable", _points_with_grid(boundary, "PROPOSED_HFIS", inputs, [10.0, 10.5, 11.0])))["PROPOSED_HFIS"]
    rough = _by_method(summarize_boundary(boundary, "favourable", _points_with_grid(boundary, "FUZZY_COMPONENT_MAX", inputs, [10.0, 20.0, 11.0])))["FUZZY_COMPONENT_MAX"]
...
def test_summarize_continuity_smoothness_does_not_overclaim_when_crisp_max_is_smoother():
    boundary = ContinuityBoundary(boundary_id="b1", channel="pm2_5", boundary_value=15.0)
    inputs = [0.0, 1.0, 2.0]
    rough = _by_method(summarize_boundary(boundary, "favourable", _points_with_grid(boundary, "PROPOSED_HFIS", inputs, [10.0, 20.0, 11.0])))["PROPOSED_HFIS"]
    smooth = _by_method(summarize_boundary(boundary, "favourable", _points_with_grid(boundary, "FUZZY_COMPONENT_MAX", inputs, [10.0, 10.5, 11.0])))["FUZZY_COMPONENT_MAX"]
